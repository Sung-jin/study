...
* 불변 객체로 벨류 타입을 구현하면 보다 안전한 코드로 작성할 수 있다
  * 불변 객체는 참조 투명성과 스레드에 안전한 특징을 가지고 있다

#### 엔티티 식별자와 벨류 타입

* 엔티티 식별자의 실제 데이터는 String 과 같은 문자열로 구성된 경우가 많다
  * 신용카드 16자리 번호나 이메일 주소 등
* 예제의 `Money` 처럼 '돈' 을 의미하는 도메인이므로 이러한 식별자는 단순한 문자열이 아닌 도메인에서 특별한 의미를 가지는 경우가 많기 때문에 식별자를 위한 벨류 타입을 사용해서 의미가 잘 드러나도록 할 수 있다

#### 도메인 모델에 set 메서드 넣지 않기

* 도메인 모델에 get/set 메서드를 무조건 추가하는 것은 좋지 않은 버릇이다
  * 특히 set 메서드는 도메인의 핵심 개념이나 의도를 코드에서 사라지게 한다
    * kotlin 을 사용하면, data class 에서 생성자에 private 을 사용하면 생성자로 처음 설정된 값에서 변경할 수 없다
    * `private var field: xxx` 와 같이 설정했으면, 내부 클래스의 함수를 통해 값을 변경할 수 있다
  * 습관적으로 코드를 작성하는 경우라면 필드값만 변경하고 끝나느 경우가 많기 때문에 상태 변경과 관련된 도메인 지식이 코드에서 사라지게 된다
* set 메서드의 또다른 문제는 도메인 객체를 생성할 때 완전한 상태가 아닐 수 있다는 점이다
* 불변 벤류 타입을 사용하면 자연스럽게 벨류 타입에는 set 메서드를 구현하지 않는다
...
// 위와 같이 주문자를 설정하는 것을 누락하게 되고, 주문자 정보를 담고 있는 필드가 null 인 상황에서 order.setState() 메서드로
// 상품 준비 중 상태로 바꾸는 시도를 한다.
// orderer 가 정상인지 확인을 위해 setState() 에서 orderer 가 null 인지 검사하는 코드를 작성하는 것도 적절하지 않다

// 즉, 도메인 객체가 불완전한 상태로 사용되는 것을 막으려면 생성 시점에 필요한 것을 전달해 주어야 한다
// 이는 생성자를 통해 필요한 데이터를 모두 받아야 한다는 것이다
// 생성자로 필요한 모든 것을 모두 받으므로, 생성자에서 필요한 데이터가 올바른지 검사할 수 있다
```

### 도메인 용어

* 도메인에서 사용하는 용어를 코드에 반영하지 않으면 해당 코드는 개발자에게 코드의 의미를 해석해야 하는 부담이 추가된다
...
* 위와 같은 형태로 상태를 구현하였다면, 해당 스텝들이 어떠한 단계인지를 파악해야 하고, 사용한 코드에서도 인지하고 로직을 파악해야 한다

```java
public void vertifyStep1OrStep2() {
...
* 또한, 스텝을 명확하게 구현하지 않았으므로, 해당 스텝을 활용한 함수나 유효검사 등의 이름도 명확하지 않은 이름으로 지어지게 된다
* 즉, 도메인 용어를 명확하게 사용하여 도메인의 중요 규칙이 잘 드러나게 활용해야 한다
  * 도메인 용어를 명확하게 사용하면 코드를 도메인 용어로 해석하거나 도메인 용어를 코드로 해석하는 과정이 줄어든다
